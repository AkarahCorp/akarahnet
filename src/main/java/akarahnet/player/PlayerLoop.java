...
public class PlayerLoop {
    ...
    public static void tick(Player p) {
        ...
        StatsHolder.getInstance().setHealth(
                p.getUniqueId(),
                StatsHolder.getInstance().getHealth(p.getUniqueId()) + stats.get(StatsObject.MAX_HEALTH) / 10000
        );

        StatsHolder.getInstance().setMana(
                p.getUniqueId(),
                StatsHolder.getInstance().getMana(p.getUniqueId()) + stats.get(StatsObject.MAX_MANA) / 10000
        );
        ...
        p.sendActionBar(
                Component.empty()
                        .append(
                                Component.text((int) hp + "/" + (int) stats.get(StatsObject.MAX_HEALTH))
                        ...
                        .append(
                                Component.text((int) mana + "/" + (int) stats.get(StatsObject.MAX_MANA))
                        ...
                        Component.empty()
                                .append(
                                        Component.text("AKARAHNET")
                                                .color(TextColor.color(200, 0, 200))
                                                .decoration(TextDecoration.BOLD, true)
                                )
                                .append(
                                        Component.text(" @ ")
                                ...
                                .append(
                                        Component.text("mc.akarah.dev")
                                                .color(TextColor.color(175, 0, 175))
                                )
                                .append(
